...
const App = () => {
  ...
  const editionDrop = useEditionDrop("0x984d0882BE3E36C6e183f4fe1Ba734d7eD89Fae7");

  //Recuperar Token
  const token = useToken("0xb72AAE7eD9568D0F23Cd8A1ba14aEe96F2E8fe16");

  //Recuperar Vote
  const vote = useVote("0x6F5A5F986545e9968C189F4Aeb0931507281A7D5");
...
const memberList = useMemo(() => {
  ...
  const mintNft = async () => {
    try {
      ...
      console.log(`🌊 Mint realizado com sucesso! Olhe na OpenSea: https://testnets.opensea.io/assets/${editionDrop.getAddress()}/0`);
      setHasClaimedNFT(true);
    } catch (error) {
      setHasClaimedNFT(false);
      console.error("Falha ao mintar NFT", error);
    } finally {
      setIsClaiming(false);
    }
  };
  ...
  return (
    <div className="mint-nft">
      <h1>Mint gratuitamente seu NFT de membro ☄️ da AvengersDAO</h1>
      <button
        disabled={isClaiming}
        onClick={mintNft}
      >
        {isClaiming ? "realizando mint..." : "Mint seu NFT (GRATIS)"}
      </button>
    </div>
  );
